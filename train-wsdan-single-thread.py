...
Modified: Nov 24,2020 - Yingqi Yu
"""
import os
import time
import logging
...
def main_worker(local_rank, ngpus_per_node, args):
    ...
    dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:50001', world_size=ngpus_per_node, rank=local_rank)
    ...
    optimizer1 = torch.optim.SGD(net.parameters(), lr=learning_rate, momentum=0.9, weight_decay=1e-5)
    optimizer2 = torch.optim.SGD(net.parameters(), lr=learning_rate/3, momentum=0.9, weight_decay=1e-5)
    optimizer3 = torch.optim.SGD(net.parameters(), lr=learning_rate/2, momentum=0.9, weight_decay=1e-5)
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.9, patience=2)
    scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer1, step_size=1, gamma=0.95)
    scheduler2 = torch.optim.lr_scheduler.StepLR(optimizer2, step_size=1, gamma=0.95)
    scheduler3 = torch.optim.lr_scheduler.StepLR(optimizer3, step_size=1, gamma=0.95)
    for epoch in range(start_epoch, config.epochs):
        logs['epoch'] = epoch + 1
        print("epoch|",logs['epoch'])
        logs['lr'] = optimizer1.param_groups[0]['lr']
        ...
        train(logs=logs,
              ...
              optimizer1=optimizer1,optimizer2=optimizer2,optimizer3=optimizer3, ngpus_per_node=ngpus_per_node, local_rank=local_rank)
        validate(logs=logs,
...
def train(**kwargs):
    ...
    for i, (X, y) in enumerate(data_loader):
        ...
        feature_center[y] =feature_center[y] +  config.beta * (feature_matrix.detach() - feature_center_batch)
        dist.all_reduce(feature_center, op=dist.ReduceOp.SUM)
        feature_center = feature_center / ngpus_per_node
        ...
        batch_loss = cross_entropy_loss(y_pred_crop, y) / 3
        batch_loss.backward(retain_graph=True)
        optimizer2.step()
        # optimizer.zero_grad()
        # batch_loss = cross_entropy_loss(y_pred_raw, y)
        ...
        batch_loss = cross_entropy_loss(y_pred_drop, y) / 2.
        batch_loss.backward(retain_graph=True)
        optimizer3.step()
        # optimizer.zero_grad()
        # loss
